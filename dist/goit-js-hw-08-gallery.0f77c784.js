...
const galleryMarkup = (0, _galleryItemsJs.galleryItems).map(({ preview, original, description: description1 })=>`
...
function onKeyPress(evt) {
    ...
    if (lightboxRef.classList.contains("is-open")) {
        const currentIndex = (0, _galleryItemsJs.galleryItems).findIndex((item)=>item.original === lightboxImgRef.src);
        if (evt.code === "ArrowRight" && currentIndex < (0, _galleryItemsJs.galleryItems).length - 1) {
            const next = (0, _galleryItemsJs.galleryItems)[currentIndex + 1];
            openModal(next.original, next.description);
        }
        if (evt.code === "ArrowLeft" && currentIndex > 0) {
            const prev = (0, _galleryItemsJs.galleryItems)[currentIndex - 1];
...
function onGalleryClick(evt) {
    ...
    if (!isImageEl) return;
    const largeImageURL1 = evt.target.dataset.source;
    const alt = evt.target.alt;
    openModal(largeImageURL1, alt);
}
...
},{"./gallery-items.js":"hpqTZ"}],"jnFvT":[function(require,module,exports,__globalThis) {
...
},{}]},["7wZbQ","2R06K"], "2R06K", "parcelRequired535", {})

//# sourceMappingURL=goit-js-hw-08-gallery.0f77c784.js.map
